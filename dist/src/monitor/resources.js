...
export function getSwapUsage() {
    try {
        ...
        if (match) {
            const toGB = (val, unit) => unit === 'G' ? parseFloat(val) : parseFloat(val) / 1024;
            return {
 ...
 */
export function getDiskUsage(mount = '/') {
    try {
 ...
 */
export function checkInstallationFeasibility(requiredDiskGB, requiredRamGB) {
    const disk = getDiskUsage();
    const ram = getRamUsage();
    const warnings = [];
 ...
 */
export function formatResourceReport(snapshot) {
