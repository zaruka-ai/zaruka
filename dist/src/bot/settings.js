...
const browsingProvider = new Map();
export function settingsText(configManager) {
...
export function settingsKeyboard(configManager) {
...
function resourcesText(configManager) {
...
function resourcesKeyboard(configManager) {
    const alertsEnabled = configManager.isResourceMonitorEnabled();
    const onOff = alertsEnabled ? t(configManager, 'settings.on') : t(configManager, 'settings.off');
    return Markup.inlineKeyboard([
...
async function showModelsForProvider(tCtx, provider, configManager) {
    ...
    if (!ai)
        return; // shouldn't happen — caller checks
    ...
    await tCtx.editMessageText(`${PROVIDER_LABELS[provider]}\n\n${t(configManager, 'settings.choose_model')}`, Markup.inlineKeyboard([
        ...modelButtons,
        [Markup.button.callback(t(configManager, 'settings.back'), 'settings:model')],
    ]));
}
export function registerSettingsCallbacks(bot, ctx) {
    ...
    bot.action(/^model:(.+)$/, async (tCtx) => {
        ...
        if (targetProvider && targetProvider !== currentProvider) {
            ...
            if (saved) {
                ...
                await tCtx.editMessageText(t(configManager, 'settings.switched', { provider: PROVIDER_LABELS[targetProvider], model }));
            }
        }
        else {
    ...
    bot.action('settings:lang', async (tCtx) => {
        ...
        await tCtx.editMessageText(t(configManager, 'settings.lang_prompt', { lang: current }), Markup.inlineKeyboard([
    ...
    for (const [key, label] of thresholdKeys) {
        bot.action(`settings:${key}`, async (tCtx) => {
            ...
            await tCtx.editMessageText(t(configManager, 'settings.threshold_prompt', { resource: label, value: String(current) }), Markup.inlineKeyboard([
    ...
    bot.action(/^thresh:(cpu|ram|disk):(\d+)$/, async (tCtx) => {
        ...
        await tCtx.editMessageText(t(configManager, 'settings.threshold_set', { resource: resource.toUpperCase(), value: String(value) }));
    });
    bot.action(/^thresh:(cpu|ram|disk):custom$/, async (tCtx) => {
        ...
        await tCtx.editMessageText(t(configManager, 'settings.threshold_custom_prompt', { resource: resourceLabel }));
    });
    // Resources sub-menu
    ...
    bot.action('settings:reset', async (tCtx) => {
        await tCtx.answerCbQuery();
        await tCtx.editMessageText(t(configManager, 'reset.warning'), Markup.inlineKeyboard([
