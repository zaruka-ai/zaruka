...
export function createHistoryTools(messageRepo) {
    return [
        tool('search_conversation_history', 'Search through past conversation history. Use this when the user asks about previous conversations, '
            ...
            if (results.length === 0) {
                return {
                    content: [{ type: 'text', text: JSON.stringify({ found: 0, message: 'No messages found matching the query.' }) }],
                };
            }
            ...
            return {
                content: [{ type: 'text', text: JSON.stringify({ found: results.length, messages: formatted }) }],
            };
        }),
        tool('get_conversation_stats', 'Get statistics about conversation history: total messages, date range, etc. '
            ...
            return {
                content: [{ type: 'text', text: JSON.stringify({ ...disk, note: 'Provide chat_id for per-chat stats' }) }],
            };
        }),
    ];
}
