 ...
 */
export async function createBestModel(ai: AiConfig): Promise<LanguageModel> {
  const provider = createProvider(ai);

  try {
    const bestId = await fetchBestModelId(ai);
    if (bestId) return provider(bestId);
  } catch (err) {
 ...
 */
async function fetchBestModelId(ai: AiConfig): Promise<string | null> {
  if (ai.provider === 'anthropic') {
...
async function fetchBestAnthropicModel(ai: AiConfig): Promise<string | null> {
  ...
  const data = await resp.json() as { data: { id: string; display_name: string; created_at: string }[] };
...
async function fetchBestOpenAIModel(ai: AiConfig): Promise<string | null> {
...
async function fetchBestChatGPTModel(ai: AiConfig): Promise<string | null> {
  if (!ai.authToken) return null;
  try {
    ...
    const data = await resp.json() as {
      models: { slug: string; priority?: number; visibility?: string }[];
    };
