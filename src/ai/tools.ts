...
const searchCache = new Map<string, { results: { title: string; url: string; snippet: string }[]; ts: number }>();
  ...
  return {
    web_search: tool({
      ...
      execute: async (args) => {
        ...
        const providers: Array<() => Promise<{ title: string; url: string; snippet: string }[]>> = [];
