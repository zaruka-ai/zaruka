...
export function registerCommands(bot: Telegraf, ctx: BotContext): void {
  ...
  bot.command('usage', async (tCtx) => {
    ...
    await tCtx.reply(
      t(configManager, 'cmd.usage_title'),
...
const PERIOD_KEYS: Record<string, UIKey> = {
  today: 'period.today',
  week: 'period.week',
  month: 'period.month',
  year: 'period.year',
...
export function registerUsageCallbacks(bot: Telegraf, ctx: BotContext): void {
  const { configManager, usageRepo } = ctx;

  bot.action(/^usage:(today|week|month|year)$/, async (tCtx) => {
    await tCtx.answerCbQuery();
    const period = tCtx.match[1] as 'today' | 'week' | 'month' | 'year';
      ...
      if (summary.requests === 0) {
        try {
          ...
          await tCtx.editMessageText(
            `${header}\n\n${t(configManager, 'cmd.usage_no_data')}`,
            keyboard,
          );
        } catch { /* ignore */ }
        return;
