...
export async function finishOnboarding(handler: OnboardingHandler, ctx: Ctx): Promise<void> {
  ...
  if (!result.ok) {
    ...
    await ctx.reply(
      ...
      Markup.inlineKeyboard([[Markup.button.callback('Retry setup', 'onboard:retry')]]),
    );
    return;
  }
...
export async function completeOnboarding(handler: OnboardingHandler, ctx: Ctx): Promise<void> {
  const state = handler.state;
  if (!state) return;
