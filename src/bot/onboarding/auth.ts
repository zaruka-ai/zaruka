...
import type { OnboardingHandler, OnboardingState, Ctx } from './handler.js';

export async function handleProviderSelected(
  handler: OnboardingHandler, ctx: Ctx, provider: AiProvider,
): Promise<void> {
...
export async function handleAuthMethod(
  handler: OnboardingHandler, ctx: Ctx, method: 'api_key' | 'oauth',
): Promise<void> {
  const state = handler.state!;
  if (!state.provider) {
    handler.state = null;
...
export async function handleApiKeyInput(
  handler: OnboardingHandler, ctx: Ctx, input: string,
): Promise<void> {
  const state = handler.state!;
    ...
    if (input.startsWith('sk-ant-oat01-')) {
      ...
      try {
        ...
        const tokens = await exchangeCodeForTokens(
          ANTHROPIC_OAUTH, code, state.codeVerifier!, state.oauthState,
        );
...
function pollDeviceCodeInBackground(
  ...
  pollDeviceToken(
    OPENAI_OAUTH, deviceAuthId, userCode, 60, 5000,
    (attempt, max, status) => {
      if (attempt % 6 === 0) {
