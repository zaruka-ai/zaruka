...
function bucketize(daily: ChartDailyData[], period?: string): Bucket[] {
  // Yearly: bucket by month
  if (period === 'year') {
    const months: Map<string, Bucket> = new Map();
  ...
  const weeks: Map<string, Bucket> = new Map();
  for (const d of daily) {
...
function formatYLabel(value: number, mode: 'tokens' | 'cost'): string {
  if (mode === 'cost') {
...
function buildSvg(buckets: Bucket[], options: ChartOptions): string {
...
function buildNoDataSvg(options: ChartOptions): string {
  const W = options.width ?? 800;
  const H = options.height ?? 400;
  return [
...
export async function generateUsageChart(
  daily: ChartDailyData[],
