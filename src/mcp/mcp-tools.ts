...
export function createMcpManagementTools(
  ...
  rebuildRef: { current: (() => Promise<void>) | null },
): ToolSet {
  return {
    add_mcp_server: tool({
      description:
        'Add (or update) an MCP server. After saving, the assistant reconnects to all MCP servers automatically. '
      ...
      execute: async (args) => {
        ...
        if (args.type === 'http' || args.type === 'sse') {
          if (!args.url) return JSON.stringify({ error: 'url is required for http/sse servers' });
          config = { type: args.type, url: args.url, headers: args.headers };
        } else {
          if (!args.command) return JSON.stringify({ error: 'command is required for stdio servers' });
        ...
        try {
          if (rebuildRef.current) await rebuildRef.current();
        } catch (err) {
    ...
    remove_mcp_server: tool({
      ...
      execute: async (args) => {
        ...
        if (!removed) return JSON.stringify({ error: `MCP server "${args.name}" not found` });
      ...
      execute: async () => {
        ...
        return JSON.stringify({
          servers: entries.map(([name, cfg]) => ({
            name,
            type: cfg.type ?? 'stdio',
            ...(cfg.type === 'http' || cfg.type === 'sse' ? { url: (cfg as { url: string }).url } : { command: (cfg as { command: string }).command }),
          })),
        });
      },
    }),
    ...
    search_mcp_servers: tool({
      description:
        'Search the official MCP server registry to find servers by keyword. '
