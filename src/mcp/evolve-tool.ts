...
import { tool, generateText, stepCountIs, type Tool, type ToolSet } from 'ai';
import { existsSync, mkdirSync, readFileSync, writeFileSync } from 'node:fs';
import { resolve } from 'node:path';
import { createBestModel, type AiConfig } from '../ai/model-factory.js';
    ...
    execute: async (args) => {
      ...
      const innerTools: ToolSet = {
        ...
        read_file: tool({
          ...
          execute: async (a) => {
            if (!existsSync(a.path)) return JSON.stringify({ error: 'File not found' });
