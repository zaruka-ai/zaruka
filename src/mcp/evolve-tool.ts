 ...
 */
async function preResearch(description: string): Promise<{ urls: string[]; snippets: string[]; pages: string[] }> {
  const urls: string[] = [];
  ...
  for (const url of urls.slice(0, 3)) {
    try {
      ...
      if (!contentType.includes('text/html') && !contentType.includes('text/plain') && !contentType.includes('application/json')) continue;
...
export function createEvolveTool(skillsDir: string, authToken?: string): SdkMcpToolDefinition<any> {
  return tool(
    'evolve_skill',
    'SELF-EVOLUTION: Create a new skill that adds capabilities you don\'t have yet. You MUST call this tool whenever a user asks for something no existing tool can do — BEFORE saying you cannot help. The new skill will be available on the next message. Be creative: use public APIs, web scraping, calculations, etc.',
    {
    ...
    async (args) => {
      ...
      for await (const msg of conversation) {
        if (msg.type === 'result') {
          if (msg.subtype === 'success' && msg.result) {
            ...
            resultText = `Error: ${(msg.errors as string[]).join(', ')}`;
          }
        }
      }
      ...
      if (created) {
        try {
          ...
          if (skillUrls.length > 0) authParts.push(`Auth/signup URLs: ${skillUrls.join(', ')}`);
          // Find env variable names
          const envMatches = content.matchAll(/process\.env\.(\w+)/g);
          const envVars = [...new Set([...envMatches].map((m) => m[1]))];
          if (envVars.length > 0) authParts.push(`Required env vars: ${envVars.join(', ')}`);
          // Find auth_methods
          const methodsMatch = content.match(/auth_methods:\s*\[([^\]]+)\]/);
          if (methodsMatch) authParts.push(`Auth methods: ${methodsMatch[1]}`);
          // Find message about auth
          const msgMatch = content.match(/message:\s*['"`]([^'"`]+)['"`]/);
          if (msgMatch) authParts.push(`Instructions: ${msgMatch[1]}`);
        } catch { /* ignore */ }
      }
